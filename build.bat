...
set py_version_minor=11
set py_version_bugfix=2
...
call .\PCBuild\get_externals.bat --organization python --no-tkinter --python %py_version_major_minor%
...
copy %builddir%\libffi-8.dll %outdir%\DLLs || exit /b 6
...
copy %builddir%\python%py_dll_major_minor%.dll %outdir% || exit /b 14
...
cd %builddir%
gendef python%py_dll_major_minor%.dll
if "%TARGET_ARCH%" == "x64" (
    dlltool -m i386:x86-64 --dllname python%py_dll_major_minor%.dll --def python%py_dll_major_minor%.def --output-lib libpython%py_dll_major_minor%.a
)
if "%TARGET_ARCH%" == "x86" (
    dlltool -m i386 --as-flags=--32 --dllname python%py_dll_major_minor%.dll --def python%py_dll_major_minor%.def --output-lib libpython%py_dll_major_minor%.a
)
copy libpython%py_dll_major_minor%.a %outdir%\libs || exit /b 18
